...
def main() -> None:
    ...
    tracking_uri = cfg.get("tracking", {}).get("uri", "mlruns")
    ...
    with mlflow.start_run(run_name=experiment_name):
        ...
        for iteration in range(1, stop_iterations + 1):
            ...
            if iteration % checkpoint_frequency == 0:
                checkpoint_dir = Path("artifacts/checkpoints") / experiment_name
                checkpoint_dir.mkdir(parents=True, exist_ok=True)
                final_checkpoint = algo.save(str(checkpoint_dir))
                checkpoint_path = Path(getattr(final_checkpoint, "path", str(final_checkpoint)))
